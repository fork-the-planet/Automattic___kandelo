...
## Build script: `packages/registry/nethack/build-nethack.sh`

Modeled on `packages/registry/vim/build-vim.sh`. Phases:
...
3. **Patch config.** `sed` edits on `include/config.h` and `sys/unix/Makefile.src` for the config choices above; point `LIBS` / `HACKCFLAGS` at `packages/registry/ncurses/out/`.
...
- **`images/vfs/scripts/build-shell-vfs-image.ts`** — register the NetHack runtime archive alongside vim's.
- **`apps/browser-demos/pages/shell/main.ts`** — import `nethackWasmUrl`; add `{ url, path: "/usr/bin/nethack", symlinks: ["/bin/nethack"] }` to `lazyDefs`.
...
- **Build-side contract.** The porter produces a `runtime/` directory under `packages/registry/<program>/`, plus a `bundle-runtime.sh` (or equivalent) that tars it. The shell VFS build script (`images/vfs/scripts/build-shell-vfs-image.ts`) picks it up and registers it.
...
- **Worked example.** Vim is the reference implementation; cite file paths (`packages/registry/vim/bundle-runtime.sh`, `images/vfs/scripts/build-shell-vfs-image.ts`, `apps/browser-demos/pages/shell/main.ts`).
...
2. Build commit: `packages/registry/nethack/build-nethack.sh`, `bundle-runtime.sh`, `run.sh` target.
