/*
 * Relaxed-precision copy of tests/libc/libc-test/src/common/mtest.h
 *
 * The only change is in checkulp(): the ULP threshold is raised from 1.5
 * to 100.0.  This catches gross breakage (NaN, inf, wrong sign, wildly
 * wrong magnitude) while tolerating the 1-2 ULP rounding errors that are
 * inherent to wasm32 soft-float (no hardware FPU rounding control).
 *
 * To use: compile with  -I libc-test-overlay  BEFORE  -I tests/libc/libc-test/src/common
 * so this header shadows the original.
 */
#include <fenv.h>
#include <float.h>
#include <math.h>
...
{
	/* Relaxed threshold: 100 ULP (vs 1.5 in the original).
	   Catches gross errors while tolerating soft-float rounding. */
	if (r == RN)
		return fabsf(d) < 100.0;
	return 1;
}
