...
//!       --revision 1 \
//!       --binary packages/registry/vim/bin/vim.wasm \
//!       --runtime-root packages/registry/vim/runtime \
//!       --runtime-prefix usr/share/vim/vim91 \
//!       --license packages/registry/vim/vim-src/LICENSE \
...
pub fn run(args: Vec<String>) -> Result<(), String> {
    ...
    if !progs.contains_key(&program) {
        return Err(format!(
            "program {program:?} has no packages/registry/{program}/package.toml \
