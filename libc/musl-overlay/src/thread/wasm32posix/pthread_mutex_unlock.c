 ...
 * unlock logic (copied verbatim from libc/musl/src/thread/pthread_mutex_unlock.c). */
#include "pthread_impl.h"

/* Must match crates/kernel/src/wasm_api.rs. */
#define SYS_PSHARED_MUTEX_UNLOCK 403
...
int pthread_mutex_unlock(pthread_mutex_t *m)
{
	if ((m->_m_type & 128) && !(m->_m_type & 4)) {
		long r = __syscall(SYS_PSHARED_MUTEX_UNLOCK, m->__u.__i[1]);
		return r < 0 ? -r : 0;
	}
